...
// --- MongoDB Connection ---
const uri = `mongodb+srv://${process.env.DB_USERNAME}:${process.env.DB_PASSWORD}@cluster0.fez2prt.mongodb.net/?retryWrites=true&w=majority`;
...
app.get("/", (req, res) => {
  res.send(" Foodian  is running successfully!");
});
...
app.post("/my-favorites", async (req, res) => {
  try {
    ...
    const review = await alldataCollection.findOne({
      _id: new ObjectId(reviewId),
    });
    if (!review) return res.status(404).json({ message: "Review not found" });

    const favoriteDoc = { userEmail, reviewId, review, favoritedAt: new Date() };
